...
async function startSession(sessionId, opts = {}) {
  ...
  // ✅ NEW (Update 29): .setantiedit — global toggle (settings-ext.js).
  // Detects a WhatsApp message edit (protocolMessage type 14 === MESSAGE_EDIT,
  // available on Baileys ^7.0.0-rc13) and announces old → new text in the
  // same chat it happened in. Best-effort: if the original text isn't in
  // recentMsgCache (expired, or it wasn't plain text to begin with), stays
  // silent rather than posting a confusing "someone edited something" ping.
  socket.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      let settingsExt;
      try { settingsExt = require('./plugins/settings-ext.js'); } catch (_) { settingsExt = null; }
      if (!settingsExt?.__getSetting('antiedit')) return;

      const m = chatUpdate.messages?.[0];
      const proto = m?.message?.protocolMessage;
      if (!proto || proto.type !== 14 /* MESSAGE_EDIT */) return;

      const editedId = proto.key?.id;
      const chatId = m.key.remoteJid;
      if (!editedId || !chatId) return;

      const cached = global.recentMsgCache.get(editedId);
      ...
      const senderJid = cached?.senderJid || m.key.participant || chatId;
    ...
    try {
      ...
      if (isStatus) {
        try {
          ...
          if (settingsExt.__getSetting('autolikestatus')) {
            // ✅ NEW: wire statusemojis (previously saved, never read) — pick
            // a random emoji from the configured pool instead of a hardcoded
            // ❤️. Falls back to ❤️ if the setting is empty/unparseable.
            const emojiPool = (settingsExt.__getSetting('statusemojis') || '❤️')
              .split(',').map(s => s.trim()).filter(Boolean);
            const reactEmoji = emojiPool[Math.floor(Math.random() * emojiPool.length)] || '❤️';
      ...
      if (body.startsWith(CMD_PREFIX)) {
        ...
        if (allCommands[cmd]) {
          ...
          // ✅ NEW: wire autotype/autorecord/autorecordtype (previously
          // saved to settings.json, never read). autorecordtype 'typing'
          // or 'recording' wins if explicitly set; otherwise falls back to
          // whichever of the two booleans is on (autorecord takes
          // priority over autotype if both are somehow on). Best-effort —
          // a presence-update failure should never block the command
          // itself from running.
          try {
            const settingsExtPresence = require('./plugins/settings-ext.js');
            const recordType = settingsExtPresence.__getSetting('autorecordtype');
            let indicator = null;
            if (recordType === 'typing' || recordType === 'recording') indicator = recordType === 'recording' ? 'recording' : 'composing';
            else if (settingsExtPresence.__getSetting('autorecord')) indicator = 'recording';
            else if (settingsExtPresence.__getSetting('autotype')) indicator = 'composing';
            if (indicator) await socket.sendPresenceUpdate(indicator, sender);
          } catch (_) {}
          try {
            // ✅ NEW: wire fontstyle (previously saved, never read). Only
            // wraps the `sock` reference handed to THIS command call — the
            // raw `socket` used by presence updates, event listeners, etc.
            // elsewhere in this file is untouched. Only transforms a plain
            // `{ text }` payload (skips captions/buttons/media messages
            // entirely, so formatting-sensitive replies aren't mangled).
            let cmdSock = socket;
            try {
              const settingsExtFont = require('./plugins/settings-ext.js');
              const style = settingsExtFont.__getSetting('fontstyle');
              if (style && style !== 'off') {
                cmdSock = new Proxy(socket, {
                  get(target, prop, receiver) {
                    if (prop === 'sendMessage') {
                      return async (jid, content, options) => {
                        if (content && typeof content.text === 'string' && Object.keys(content).length === 1) {
                          content = { ...content, text: settingsExtFont.__applyFontStyle(content.text, style) };
                        }
            ...
            // Full error always still goes to console + the panel activity
            // log either way — antibug only controls what the *sender* sees.
            logActivity('error', cmd, `*.${cmd}* failed: ${e.message}`, actorTag);
            let hideDetail = false;
            try { hideDetail = require('./plugins/settings-ext.js').__getSetting('antibug'); } catch (_) {}
            try {
              await socket.sendMessage(sender,
                { text: hideDetail
                    ? `❌ Something went wrong running *.${cmd}*. Try again in a moment.`
                    : `❌ Error in .${cmd}: ${e.message}` },
                { quoted: msg }
              );
            } catch (_) {}
          }
