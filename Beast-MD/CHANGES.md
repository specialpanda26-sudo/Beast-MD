...
## Update 30 — bot self-identity: answers "who made you"/"what's your name" reliably

- **`app.py`:** new shared `IDENTITY_BLOCK` (name: Beast MD, creator: Henry Ochieng /
  henrytech254, what it does) appended to the default-DM and group AI chat personas, alongside
  the existing `SECRECY_GUARD`. These are two different things — SECRECY_GUARD hides internals
  (system prompt, keys, config); IDENTITY_BLOCK is the opposite, concrete public facts the bot
  should state plainly instead of the model improvising or getting cagey. Not added to the
  `owner_sheng` persona (Henry's own number's away-reply) since that one intentionally never
  claims to be a bot at all — adding it there would contradict that persona's whole point.
- **`plugins/general.js`:** new `.creator` command (aliases `.whoami`, `.developer`, `.botinfo`)
  — same facts, but instant and free (no Groq call), for when someone wants a direct answer
  rather than a conversational one. Kept in sync with the app.py block manually, since they're
  different runtimes/languages.
- **Assumption flagged:** both are currently hardcoded to Henry Ochieng / Beast MD, matching
  the branding already baked in elsewhere (menu footer, etc.). If you're running separate
  sessions for other people/brands off this same codebase, this identity would say "Henry" on
  all of them — say the word and I'll make it read from `config.botOwner`/`BOT_NAME` per-session
  instead of the shared hardcoded text.

**"Personal section" concept (from conversation with the owner):** once someone has this bot's
number saved/messaged, everything already tied to their number should be visible in one place.

- **New `/whatsapp-profile` route (`app.py`)** — aggregates registration credits/badge,
  referral count/earnings, and how many `.remember` notes exist in that chat, keyed by phone.
- **New `.myspace`** (aliases `.mysection`, `.myaccount`, `plugins/general.js`) — the one-stop
  view: registration status, credits/badge, referral stats, remembered-note count, current AI
  model for that chat.
- **New `.referral`** — turns out this was documented in the README (".referral gets your
  link") but never actually built as a command, even though the backend referral system
  (bonus payouts, audit table) already worked. Built it: code = the referrer's own phone number,
  matching exactly what `/api/register`'s existing `ref` field already expects.
- **Found, flagged, not fixed:** `register.html`'s signup form doesn't read a `?ref=` URL param
  or send it to the backend at all — so a shared referral link is correct but won't actually
  credit anyone until that frontend piece is wired up. Said so directly in the `.referral` reply
  text too, rather than silently shipping a link that looks like it works but doesn't.



## Update 29 — antisticker/antistickerkick/antistickerwarn, antiedit, fontstyle wired; dead duplicates removed

Finishes the remaining `settings-ext.js` toggles from Update 28's "still open" list.

**New (not just wiring — these needed real detection logic that didn't exist):**
- **`.setantisticker`** (+ `.setantistickerkick` / `.setantistickerwarn`) — added to
  `groupguard.js`'s `_enforceGroupGuard` (same hook that already handles antilink/antiflood/
  badwords/antimention). Deliberately kept as a *global* toggle rather than per-group like those,
  since it's meant as a bot-wide policy. Kick wins over warn if both are on. Note: this hook has
  a pre-existing bug (not introduced here, not fixed here) — `_enforceGroupGuard` is called
  without `groupMeta`, so its real-admin exemption never actually triggers for any of the rules
  in this function, antisticker included. Flagging, not silently fixing, since that's a
  behavior change to *existing* rules beyond what was asked.
- **`.setantiedit`** — new `messages.upsert` listener in `client_bridge.js`, sibling to the
  existing `.antidelete` one, detecting `protocolMessage.type === 14` (MESSAGE_EDIT, supported
  on the pinned Baileys `^7.0.0-rc13`). Announces before/after text in the same chat, reusing
  the same `recentMsgCache` the delete-recovery feature already populates. Best-effort/silent if
  the original text isn't cached.
- **`.setfontstyle`** — new `applyFontStyle()` unicode transform in `settings-ext.js` (bold,
  italic, bolditalic, mono, sans, sansbold — contiguous Unicode blocks only; script/fraktur/
  double-struck skipped, they have letter-by-letter exceptions that'd need a full lookup table).
  Wired via a `Proxy` wrapping only the `sock` reference handed to the command being run —
  transforms plain `{ text }` payloads only, so captions/buttons/media messages aren't touched.
  The raw `socket` used elsewhere (presence, listeners) is untouched.

**Removed, not wired** — per Update 28's flag, these were dead duplicates of features that
already exist and work under a different name:
- `.setantidelete` / `.setstatusantidelete` (global, no-op) — the real, working command is
  `.antidelete on/off` (per-chat, `extended.js`, backed by `apiClient`/`recentMsgCache`). Kept
  that one, deleted these two, including their now-dangling admin-panel rows in `admin.html`.
- `.setwarnlimit` (global, no-op) — real per-feature limits already exist and work
  (`.setfloodkick`, `.setantigcmentionwarnlimit` in `groupguard.js`). Deleted, including its
  admin-panel row.



## Update 28 — wired 4 more dead settings toggles (antibug, autotype, autorecord, statusemojis)

Continuing the Update 17 fix (which wired autoread/autoreact/chatbot/etc. into real behavior) for
the remaining `settings-ext.js` toggles that still just saved to `data/settings.json` and did
nothing. Wired in this pass, at the single command-dispatch choke point in `client_bridge.js`:

- **`.setantibug`** — when ON, command errors sent to the user are now a generic "something went
  wrong" instead of the raw `e.message`. Full error still always goes to console + the panel
  activity log either way — this only controls what the *sender* sees. Default stays OFF (raw
  error text), matching prior behavior.
- **`.setautotype` / `.setautorecord` / `.setautorecordtype`** — now sends a real "typing…" or
  "recording audio…" presence update right before a command runs. `autorecordtype` (`typing` /
  `recording`) wins if explicitly set; otherwise falls back to whichever of the two booleans is
  on. Best-effort — wrapped so a presence-update failure never blocks the command itself.
- **`.setstatusemojis`** — `autolikestatus`'s status-react now picks a random emoji from this
  comma-separated pool instead of a hardcoded ❤️. Falls back to ❤️ if the pool is empty.

**Still open, deliberately not touched in this pass** — these need new detection logic, not just
reading a setting back, so they're a different size of job:
- `.setantiedit` — no message-edit listener exists anywhere yet.
- `.setantisticker` / `.setantistickerkick` / `.setantistickerwarn` — no incoming-sticker
  detection in groups exists yet.
- `.setantidelete` / `.setstatusantidelete` — **do not confuse these with the working
  `.antidelete on/off`** in `extended.js`, which is a real, separate, per-chat command backed by
  `apiClient`/`recentMsgCache`. The `settings-ext.js` global versions of these two are dead
  duplicates with confusingly similar names; recommend removing them rather than wiring them, to
  stop two different toggles from claiming to do the same thing.
- `.setwarnlimit` — also a likely-dead duplicate: real per-feature warn thresholds already exist
  and work (`.setfloodkick`, `.setantigcmentionwarnlimit` in `groupguard.js`). Same
  recommendation — remove rather than wire.
- `.setfontstyle` — would need a global outgoing-text wrapper (transform every `sendMessage`
  call), which is a more invasive change than a single wiring point; flagging for a deliberate
  decision rather than doing it inline here.
- `levelup` — already had exactly 1 real usage found in the codebase; confirmed already wired,
  nothing to do.
...
voluntary "Buy Me a Coffee" donate link stays. This removes the entire `.pair key` → admin
approve/deny → `.key XXXXXX` gated-access flow added in Update 25's "sub-admin activation" work,
along with the customer-subscription-expiry half of it.

**Removed:**
- `client_bridge.js`: `sessionActivation` lock state, `getActivation`/`isSessionLive` helpers, the
  entire approve/deny/`.pair key`/`.key`/locked-session-notice block, and the activation-status
  check on new pairing. The free self-service `.pair` (link your own number, right in chat) is
  untouched and works exactly as before — it was never part of the paid system.
- `app.py`: the whole "Paid Pairing / Activation Keys" section — `session_subscriptions` gating
  logic and all `/admin/activation-*` routes (`status`, `request`, `approve`, `deny`, `redeem`,
  `extend`, `list`, `settings`).
- `plugins/general.js`: `.extend <days>` command (the customer-subscription-extension half of the
  same system).
- `admin.html`: the **Access Requests** tab (approve/deny/extend UI, key reveal) and the
  **Activation Defaults** (default days / bypass key) block under Settings.
- `assets/commands-db.json`: stale `.extend` entry.
- Doc generators (`generate-user-guide-pdf.py`, `generate-whats-fixed-pdf.py`): rewritten to
  describe instant free pairing instead of the key-redemption flow.

**Kept, deliberately untouched:** the simple owner-set manual expiry (`/admin/set-expiry`,
`/admin/check-terminate`, in-memory `SESSION_REGISTRY`) — this is optional owner tooling to
manually cut off one specific session (e.g. an arrangement made outside the bot entirely), not an
automated payment/lock system, and was never part of what was flagged for removal. The
`payments` (wallet top-up) SQLite table was already dead code — created but never read from or
written to by any route — since the `.addfunds`/`.paypalfunds`/`.profile`/`.referral` commands it
was meant to back were never actually implemented as real commands.

## Update 25 — Extended commands + sub-admin activation
...
**Backend additions:** 7 new tables (`group_activity`, `group_relations`, `polls`, `poll_votes`, `reports`, `group_bans`, `chat_settings`) and ~30 new routes, all additive. `admin.html` already has matching **Reports** and **Group Bans** tabs (`pane-reports`/`pane-groupbans`) wired to `loadReports`/`loadGroupBans` and the resolve/unban actions — this was a real gap when first written but has since been closed.
