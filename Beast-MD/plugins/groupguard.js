...
module.exports = {
  ...
  _enforceGroupGuard: async ({ sock, from, msg, body, sender, isGroup, groupMeta }) => {
    ...
    // ✅ NEW (Update 29): .setantisticker — global toggle (settings-ext.js),
    // deliberately NOT per-group like the settings above, since it's meant
    // as a bot-wide policy rather than something each group opts into
    // separately. Deletes any incoming sticker in a group; antistickerkick
    // removes the sender, antistickerwarn just posts a warning instead
    // (kick wins if both are somehow on).
    if (msg.message?.stickerMessage) {
      let settingsExt;
      try { settingsExt = require('./settings-ext.js'); } catch (_) { settingsExt = null; }
      if (settingsExt?.__getSetting('antisticker')) {
        try { await sock.sendMessage(from, { delete: msg.key }); } catch (e) {}
        if (settingsExt.__getSetting('antistickerkick')) {
          try {
            await sock.groupParticipantsUpdate(from, [sender], 'remove');
            await sock.sendMessage(from, { text: `🖼️ @${sender.split('@')[0]} removed for posting a sticker in this group.`, mentions: [sender] });
          } catch (e) {
            await sock.sendMessage(from, { text: `🖼️ @${sender.split('@')[0]}, stickers aren't allowed here.`, mentions: [sender] }).catch(() => {});
          }
        } else if (settingsExt.__getSetting('antistickerwarn')) {
          try { await sock.sendMessage(from, { text: `🖼️ @${sender.split('@')[0]}, stickers aren't allowed here.`, mentions: [sender] }); } catch (e) {}
        }
        return true;
      }
    }
