...
const DEFAULTS = {
  ...
  fontstyle: 'off',          // 'off'|'bold'|'italic'|'bolditalic'|'mono'|'sans'|'sansbold' — applied to plain-text bot replies sent via commands
};
...
const BOOL_KEYS = ['pmpermit','chatbot','autoreact','autoread','autoreadstatus','autobio','autoreply','autoreplystatus','autoblock','autolikestatus','firstcontactgreeting','levelup',
  'anticall','antibug','antiedit','antisticker','antistickerkick','antistickerwarn','autotype','autorecord'];
// ✅ Update 29: dropped 'antidelete', 'statusantidelete', 'warnlimit' from here — they were
// dead duplicates of features that already exist and work under a different mechanism
// (`.antidelete on/off` in extended.js; `.setfloodkick`/`.setantigcmentionwarnlimit` in
// groupguard.js). Removing the confusing look-alike rather than wiring a second copy of
// something that already works. See CHANGES.md Update 29.
const TEXT_KEYS = ['botname','prefix','dmpresence','gcpresence','welcomemessage','goodbyemessage','ownername','firstcontactmessage',
  'anticallmessage','statusemojis','autorecordtype','fontstyle'];
...
// ── Update 29: .setfontstyle — transforms plain ASCII letters/digits into a
// unicode "styled" variant. Only contiguous, exception-free Unicode blocks
// are supported (bold/italic/bolditalic/mono/sans/sansbold) — styles like
// script/fraktur/double-struck have irregular gaps (a handful of letters
// borrow from a different block entirely) and were left out rather than
// shipping a transform with silent wrong characters for those letters.
const FONT_OFFSETS = {
...
function applyFontStyle(text, style) {
  ...
  return [...text].map(ch => {
    ...
    if (map.digit && code >= 48 && code <= 57) return String.fromCodePoint(map.digit + (code - 48));
    return ch;
  }).join('');
}
...
const exportsObj = {
  ...
  __applyFontStyle: applyFontStyle,
};
