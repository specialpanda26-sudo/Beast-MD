...
module.exports = {
  ...
  // ── .creator / .whoami / .developer / .botinfo ──────────────────────────
  // ✅ NEW (Update 29): instant, deterministic identity answer — no Groq
  // call needed, so it always answers correctly and costs nothing, unlike
  // relying on the AI persona to remember to mention this on its own.
  // Same facts as the IDENTITY_BLOCK now appended to the AI chat personas
  // in app.py, kept in sync manually since they live in different runtimes.
  creator: async ({ sock, from, msg, config }) => {
    const botName = config?.botName || process.env.BOT_NAME || 'Beast MD';
    const text = `🤖 *${botName}*\n\n`
      + `👨‍💻 Built by: Henry Ochieng (GitHub: henrytech254)\n`
      + `🧩 What I do: AI chat, media downloads, stickers, games, group moderation, and more — run *.menu* to see everything.\n\n`
      + `Ask me anything else about myself and I'll answer in chat too.`;
    await sock.sendMessage(from, { text }, { quoted: msg });
  },
  whoami: async (h) => module.exports['creator'](h),
  developer: async (h) => module.exports['creator'](h),
  botinfo: async (h) => module.exports['creator'](h),

  // ── .myspace / .mysection / .myaccount ──────────────────────────────────
  // ✅ NEW (Update 30): "personal section" concept from the owner — once
  // someone has this bot's number saved/messaged, everything already tied
  // to their number (registration credits/badge, referral stats, saved
  // .remember notes, AI model choice) shown in one place, instead of
  // scattered across separate commands with no single "this is your space"
  // view. Backed by the new /whatsapp-profile route in app.py.
  myspace: async ({ sock, from, msg, senderJid, apiClient, config }) => {
    const phone = senderJid.split('@')[0].replace(/:\d+$/, '');
    const botName = config?.botName || process.env.BOT_NAME || 'Beast MD';
    let data = null;
    try {
      const res = await apiClient.get('/whatsapp-profile', { params: { phone, chat_id: from } });
      data = res?.data;
    } catch (_) { /* fall through to the "couldn't load" message below */ }

    if (!data) {
      return sock.sendMessage(from, { text: `❌ Couldn't load your space right now — try again in a moment.` }, { quoted: msg });
    }

    const model = global.chatModel?.get(from) || 'default';
    const lines = [
      `📂 *Your space on ${botName}*`,
      ``,
      data.registered
        ? `👤 Registered: ✅ (${data.verified ? 'verified' : 'not yet verified'})\n💰 Credits: ${data.credits}\n🛡️ Badge: ${data.badge}`
        : `👤 Not registered on the web panel yet — *.register* on the site unlocks starter credits + a trust badge.`,
      ``,
      `🤝 Referrals: ${data.referral_count} signed up, ${data.referral_earned} kesh earned. *.referral* for your link.`,
      ``,
      `🧠 Notes saved here: ${data.remembered_count} (*.remember*/*.recall*)`,
      `🤖 AI model for this chat: ${model} (*.model* to change)`,
    ];
    await sock.sendMessage(from, { text: lines.join('\n') }, { quoted: msg });
  },
  mysection: async (h) => module.exports['myspace'](h),
  myaccount: async (h) => module.exports['myspace'](h),

  // ── .referral ────────────────────────────────────────────────────────────
  // ✅ NEW (Update 30): the backend referral system (registrations.referred_by,
  // the referrals audit table, /api/register's `ref` handling) already
  // existed and worked, but nothing generated the actual shareable
  // code/link — README documented ".referral gets your link" but the
  // command itself was never built. Referral code = the referrer's own
  // phone number (matches exactly what /api/register already expects in
  // its `ref` field).
  // ⚠️ Flagging, not fixing here: register.html's signup form doesn't
  // currently read a `?ref=` URL param or send it as `ref` in the request
  // — so even with this link, a referred signup won't actually get linked
  // to the referrer until that frontend piece is wired up too.
  referral: async ({ sock, from, msg, senderJid }) => {
    const phone = senderJid.split('@')[0].replace(/:\d+$/, '');
    const siteUrl = process.env.SITE_URL || '<your deployed site URL — set SITE_URL to enable a full link>';
    const text = `🤝 *Your referral code:* ${phone}\n\n`
      + `Share this link: ${siteUrl}/register.html?ref=${phone}\n\n`
      + `You get 15 kesh, they get 30 kesh, once they sign up and verify.\n\n`
      + `⚠️ Heads up: the site's registration page doesn't wire the *?ref=* link into the signup request yet — the code above is correct, but ask me to finish that piece if you want it actually crediting referrals end-to-end.`;
    await sock.sendMessage(from, { text }, { quoted: msg });
  },
