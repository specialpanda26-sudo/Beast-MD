...
module.exports = {
  ...
  // ── .quotly ─── Turn a replied text message into a styled quote-card sticker | usage: .quotly (reply to a message)
  // Renders the quoted message as a name + avatar + text card, the way
  // Telegram/WA "quotly" bots do it. Uses the public bot.lyo.su render API
  // (the same open, widely-used endpoint most WA quotly plugins call) so we
  // don't need a local canvas/font stack — just axios + wa-sticker-formatter.
  quotly: async ({ sock, from, msg }) => {
    const ctx = msg.message?.extendedTextMessage?.contextInfo;
    const quotedMessage = ctx?.quotedMessage;
    if (!ctx || !quotedMessage) {
      return sock.sendMessage(from, { text: '❌ Reply to a text message with .quotly to turn it into a quote sticker.' }, { quoted: msg });
    }
    ...
    if (!quotedText.trim()) {
      return sock.sendMessage(from, { text: '❌ That message has no text to quote (only .quotly plain text/captions).' }, { quoted: msg });
    }

    const quotedJid = ctx.participant || from;
    const quotedNumber = quotedJid.split('@')[0];

    // Try to recover the real display name from the local message store
    // (keeps pushName per message) before falling back to the raw number.
    let displayName = quotedNumber;
    try {
      const store = require('../lib_ported/lightweight_store.js');
      const chatMsgs = Array.isArray(store.messages?.[from]) ? store.messages[from] : [];
      const stored = chatMsgs.find(m => m.key?.id === ctx.stanzaId);
      if (stored?.pushName?.trim()) displayName = stored.pushName.trim();
    } catch { /* store lookup is best-effort only */ }

    let avatarUrl = 'https://cdn.jsdelivr.net/gh/WhiskeySockets/[email protected]/Media/default-avatar.png';
    try {
      avatarUrl = await sock.profilePictureUrl(quotedJid, 'image');
    } catch { /* private/no pic — keep the default avatar */ }
    ...
    try {
      ...
      const { data } = await axios.post('https://bot.lyo.su/quote/generate', {
        ...
        messages: [{
          entities: [],
          avatar: true,
          from: { id: 1, name: displayName, photo: { url: avatarUrl } },
          text: quotedText,
          replyMessage: {},
        }],
      }, { timeout: 15000 });
      ...
      const { writeExifImg } = require('../lib_ported/exif.js');
      const tmpPath = await writeExifImg(imgBuffer, { packname: 'Beast MD', author: displayName });
      await sock.sendMessage(from, { sticker: fs.readFileSync(tmpPath) }, { quoted: msg });
      fs.unlinkSync(tmpPath);
    } catch (e) {
      await sock.sendMessage(from, { text: `❌ Quotly failed (render service may be down): ${e.message}` }, { quoted: msg });
    }
  },

  q: async (h) => module.exports.quotly(h),
