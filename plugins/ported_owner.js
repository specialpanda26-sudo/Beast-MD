...
Object.assign(module.exports, (() => {
  const store = require('../lib_ported/lightweight_store.js');
  // ✅ FIX (real bug, found during a full autoread audit): this command used
  // to read/write its own separate file (data/autoread.json) or DB key
  // ('global'/'autoread' via lightweight_store) — completely disconnected
  // from the actual read-receipt gate in client_bridge.js, which checks
  // settings-ext.js's __getSetting('autoread') instead. Result: running
  // `.autoread off` here reported success but the bot kept marking every
  // message read anyway, and `.autoread on` did nothing either — pure
  // placebo. Now this command reads/writes the SAME setting settings-ext.js
  // and client_bridge.js actually use, so it does what it says.
  const settingsExt = require('./settings-ext.js');
  function isAutoreadEnabled() { return !!settingsExt.__getSetting('autoread'); }
  ...
  return {
    ...
    "autoread": async (h) => {
      ...
      try {
        ...
        const channelInfo = context.channelInfo || {};
        try {
            const enabledNow = isAutoreadEnabled();
            ...
            if (!action) {
                ...
                await sock.sendMessage(chatId, {
                    text: `*📖 AUTOREAD STATUS*\n\n` +
                        `*Current Status:* ${enabledNow ? '✅ Enabled' : '❌ Disabled'}\n` +
                        `*Stealth Mode:* ${ghostActive ? '👻 Active (overrides autoread)' : '❌ Inactive'}\n\n` +
                        `*Commands:*\n` +
                        `• \`.autoread on\` - Enable auto-read\n` +
                        `• \`.autoread off\` - Disable auto-read\n\n` +
                        `*What it does:*\n` +
                        `When enabled, the bot automatically marks all messages as read (blue ticks) — whether the account is online or offline.\n\n` +
            ...
            if (action === 'on' || action === 'enable') {
                ...
                settingsExt.__setSetting('autoread', true);
                const ghostMode = await store.getSetting('global', 'stealthMode');
                const ghostActive = ghostMode && ghostMode.enabled;
                await sock.sendMessage(chatId, {
                    text: `✅ *Auto-read enabled!*\n\nAll messages will now be automatically marked as read, online or offline.${ghostActive ? '\n\n⚠️ *Note:* Ghost mode is currently active and will override autoread.' : ''}`,
                    ...channelInfo
                }, { quoted: message });
            }
                ...
                settingsExt.__setSetting('autoread', false);
                await sock.sendMessage(chatId, {
