...
const DEFAULTS = {
  ...
  levelup: true,            // announce in-chat when a member's activity level increases
  ...
  // (see per-key notes), so wiring this in doesn't change behavior for
  // anyone until the owner deliberately visits the panel and changes one.
  anticall: true,            // matches existing unconditional call-reject behavior
  anticallmessage: '',       // '' = no follow-up text sent after rejecting a call
  antibug: false,            // hide internal error text from command replies
  antiedit: false,           // announce when a group message is edited
  antidelete: false,         // announce/resend when a group message is deleted
  antisticker: false,        // delete incoming stickers in groups
  antistickerkick: false,    // ...and remove the sender (requires antisticker ON)
  antistickerwarn: false,    // ...or just warn the sender (requires antisticker ON)
  statusantidelete: false,   // keep a copy of status updates the poster deletes
  statusemojis: '❤️,😍,😂,😮,😢,🙏', // comma-separated reaction pool for autolikestatus
  autotype: false,           // show "typing…" briefly before each command reply
  autorecord: false,         // show "recording audio…" briefly before each reply
  autorecordtype: 'off',     // 'off' | 'typing' | 'recording' — which indicator autorecord uses
  warnlimit: 3,              // generic warn threshold, surfaced in the panel for reference
  fontstyle: '',             // '' = normal text; optional unicode font applied to bot replies
};
...
const BOOL_KEYS = ['pmpermit','chatbot','autoreact','autoread','autoreadstatus','autobio','autoreply','autoreplystatus','autoblock','autolikestatus','firstcontactgreeting','levelup',
  'anticall','antibug','antiedit','antidelete','antisticker','antistickerkick','antistickerwarn','statusantidelete','autotype','autorecord'];
const TEXT_KEYS = ['botname','prefix','dmpresence','gcpresence','welcomemessage','goodbyemessage','ownername','firstcontactmessage',
  'anticallmessage','statusemojis','autorecordtype','warnlimit','fontstyle'];
...
const exportsObj = {
  ...
  // ── Update 18: generic read/write used by the Admin Panel HTTP routes
  // (client_bridge.js: GET/POST /admin/bot-settings), so every key above
  // is editable from the browser, not just via WhatsApp .set<key> commands.
  __getAllSettings: () => ({ ...loadSettings() }),
  __setSetting: (key, value) => {
    if (!(key in DEFAULTS)) return { ok: false, error: `unknown setting: ${key}` };
    const s = loadSettings();
    if (BOOL_KEYS.includes(key)) {
      s[key] = (value === true || value === 'true' || value === 'on' || value === 1);
    } else {
      s[key] = String(value ?? '');
    }
    saveSettings(s);
  ...
  __settingsMeta: () => ({ bool: [...BOOL_KEYS], text: [...TEXT_KEYS], defaults: { ...DEFAULTS } }),
};

BOOL_KEYS.forEach(k => { exportsObj[`set${k}`] = makeBoolSetter(k); });
TEXT_KEYS.forEach(k => { exportsObj[`set${k}`] = makeTextSetter(k, k); });
// ✅ FIX: welcomemessage/goodbyemessage were generic TEXT_KEYS setters that
// saved successfully but were never read by anything — the real, working
// welcome/goodbye system (per-group, fires on actual join/leave) is
// .welcomecfg (lib_ported/welcome.js). Rather than leave a command that
// looks like it works but silently does nothing, point owners at the real
// one instead of saving to a dead key.
exportsObj.setwelcomemessage = async ({ sock, from, msg }) => {
  await sock.sendMessage(from, { text: '📝 Use `.welcomecfg set <message>` instead — that\'s the one that actually controls what new members see.' }, { quoted: msg });
};
exportsObj.setgoodbyemessage = async ({ sock, from, msg }) => {
  await sock.sendMessage(from, { text: '📝 Use `.welcomecfg` to manage this group\'s leave message — that\'s the one that actually fires when someone leaves.' }, { quoted: msg });
};
