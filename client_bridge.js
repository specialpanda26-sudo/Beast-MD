...
// ── Crash resilience ─────────────────────────────────────────────────────────
// This process holds the live WhatsApp socket 24/7. Every command already
// runs inside a try/catch (see the dispatcher below), but a stray error
// outside that path — a background timer, a webhook handler, a promise
// nobody awaited — would otherwise kill the entire process and drop the
// session, forcing a re-pair. These two handlers are the last line of
// defense: log loudly, keep running, instead of dying silently.
process.on('uncaughtException', (err) => {
  console.error('🛑 [uncaughtException] kept process alive:', err?.stack || err);
});
...
function getOwnerSessionSocket() {
  ...
  // GET /internal/bot-settings, POST /internal/bot-settings/set — called
  // locally by app.py's /admin/bot-settings routes (browser auth is checked
  // there via ADMIN_PASSWORD; this route itself just checks it's a local
  // call from app.py, same as /notify-owner above). Backs the Admin Panel →
  // Settings → Feature Toggles section (Update 18).
  if (req.method === "GET" && url.pathname === "/internal/bot-settings") {
    if (!isInternalCall(req)) {
      res.writeHead(403, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ success: false, error: "forbidden" }));
    }
    try {
      ...
      res.end(JSON.stringify({
        success: true,
        settings: settingsExt.__getAllSettings(),
        meta: settingsExt.__settingsMeta(),
      }));
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ success: false, error: e.message }));
  ...
  if (req.method === "POST" && url.pathname === "/internal/bot-settings/set") {
    if (!isInternalCall(req)) {
      res.writeHead(403, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ success: false, error: "forbidden" }));
    }
    ...
    req.on("end", () => {
      try {
        ...
        const settingsExt = require('./plugins/settings-ext.js');
        const result = settingsExt.__setSetting(key, value);
...
async function startSession(sessionId, opts = {}) {
  ...
  try {
    // ✅ Tightened for "always reads immediately" behavior: owner wants
    // read receipts to feel instant whether the bot is online or offline.
    // A perfectly literal 0ms read is itself a bot fingerprint, so we keep
    // a tiny human-like jitter (roughly 80-400ms) instead of the library's
    // default up-to-8-second delay.
    socket = readReceiptVariance({ meanMs: 150, stdDevMs: 80, minMs: 80, maxMs: 400 }).wrap(socket);
  } catch (e) {
  ...
  // ✅ NEW: .antiedit — announces in-chat when a message is edited, showing
  // old vs new text. Baileys internally converts a WhatsApp message-edit
  // into a protocolMessage (type MESSAGE_EDIT) and re-emits it through
  // "messages.update" as `update.message.editedMessage.message` (confirmed
  // against Baileys' own process-message.ts) — NOT through messages.upsert
  // like a delete is, so this is its own listener. Global toggle (matches
  // settings-ext.js's existing `antiedit` key / .setantiedit), groups only,
  // per its original description in settings-ext.js.
  socket.ev.on("messages.update", async (updates) => {
    try {
      const settingsExt = require('./plugins/settings-ext.js');
      if (!settingsExt.__getSetting('antiedit')) return;
      for (const u of updates) {
        ...
        const editedId = u.key?.id;
        const cached = editedId ? global.recentMsgCache.get(editedId) : null;
        const oldText = cached?.msg?.message?.conversation
          || cached?.msg?.message?.extendedTextMessage?.text || null;
        const newText = edited.conversation || edited.extendedTextMessage?.text || null;
        if (!newText) continue; // media-only edits: nothing text-based to show
        const who = cached?.senderJid ? `+${cached.senderJid.split('@')[0]}` : (u.key?.participant ? `+${u.key.participant.split('@')[0]}` : 'Someone');
        const text = oldText
          ? `✏️ *${who} edited a message:*\n\n_Before:_ ${oldText}\n_After:_ ${newText}`
          : `✏️ *${who} edited a message:*\n\n_After:_ ${newText}`;
        await socket.sendMessage(chatId, { text });
      }
    } catch (_) { /* antiedit is best-effort, never throw into Baileys' event loop */ }
  });

  // ✅ NEW: .statusantidelete — same recovery pattern as .antidelete, for
  // statuses instead of chat messages. Text statuses only (media statuses
  // are already covered by the separate "auto-save status media to disk"
  // feature above); recovered text goes privately to the bot's own number,
  // never reposted publicly, same privacy rule the other recovery features
  // follow.
  socket.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      const m = chatUpdate.messages?.[0];
      const proto = m?.message?.protocolMessage;
      if (!proto || proto.type !== 0 /* REVOKE */) return;
      if (m.key.remoteJid !== 'status@broadcast') return;
      const settingsExt = require('./plugins/settings-ext.js');
      if (!settingsExt.__getSetting('statusantidelete')) return;
      const deletedId = proto.key?.id;
      if (!deletedId) return;
      const cached = global.recentMsgCache.get(deletedId);
      if (!cached?.isStatus) return;
      const text = cached.msg?.message?.conversation || cached.msg?.message?.extendedTextMessage?.text || null;
      if (!text) return;
      const who = cached.senderJid ? `+${cached.senderJid.split('@')[0]}` : 'someone';
      const ownNumber = socket.user?.id?.replace(/:.*@/, '@') || null;
      if (ownNumber) {
        await socket.sendMessage(ownNumber, { text: `🗑️ Deleted status from ${who}: "${text}"` });
      }
    } catch (_) { /* statusantidelete is best-effort, never throw into Baileys' event loop */ }
  });

  // ✅ NEW: .antisticker (+antistickerkick / antistickerwarn) — deletes
  // stickers posted in groups by non-admins. Reuses the SAME group_warnings
  // 3-strike backend the existing antilink feature already calls
  // (POST /antilink/strike) — that endpoint is generic (keyed by
  // group_id+sender, not link-specific), and .warn already reuses it the
  // same way, so this isn't a new system to maintain.
  socket.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      const m = chatUpdate.messages?.[0];
      if (!m?.message?.stickerMessage) return;
      const chatId = m.key.remoteJid;
      if (!chatId || !chatId.endsWith('@g.us') || m.key.fromMe) return;
      const settingsExt = require('./plugins/settings-ext.js');
      if (!settingsExt.__getSetting('antisticker')) return;
      const senderJid = m.key.participant || m.key.remoteJid;
      try {
        const groupMeta = await socket.groupMetadata(chatId);
        const participant = groupMeta.participants?.find(p => p.id === senderJid);
        if (participant?.admin) return; // never moderate group admins
      } catch (_) { /* if metadata lookup fails, fall through and still moderate */ }
      try { await socket.sendMessage(chatId, { delete: m.key }); } catch (_) {}
      const wantKick = settingsExt.__getSetting('antistickerkick');
      const wantWarn = settingsExt.__getSetting('antistickerwarn');
      if (!wantKick && !wantWarn) return;
      const strikeRes = await apiClient.post("/antilink/strike", { group_id: chatId, sender: senderJid });
      const { count = 1, kick = false } = strikeRes.data || {};
      const who = `@${senderJid.split('@')[0]}`;
      if (wantKick && kick) {
        await socket.sendMessage(chatId, { text: `🚫 ${who} removed after ${count} sticker warnings.`, mentions: [senderJid] });
        try { await socket.groupParticipantsUpdate(chatId, [senderJid], 'remove'); } catch (_) {}
      } else if (wantWarn) {
        await socket.sendMessage(chatId, { text: `⚠️ ${who} stickers aren't allowed here (${count}/3).`, mentions: [senderJid] });
      }
    } catch (_) { /* antisticker is best-effort, never throw into Baileys' event loop */ }
  });
  ...
  if (usePairingCode && !state.creds.registered) {
    // ⏪ REVERTED: the 1200ms speedup fired requestPairingCode() before the
    // socket had actually finished connecting to WhatsApp's servers on
    // slower connections (e.g. Termux over mobile data) — the code would
    // "generate" locally but WhatsApp's servers never received the
    // registration request, so the phone never got the "someone is trying
    // to link your device" prompt. Back to the safe, proven value.
  ...
  // Feature: Anti-Call — gated by settings-ext's 'anticall' (default true,
  // matching the previous unconditional behavior), with an optional
  // follow-up text set via Admin Panel → Settings → Anti Call Message.
  socket.ev.on("call", async (inboundCall) => {
    let settingsExtCall;
    try { settingsExtCall = require('./plugins/settings-ext.js'); } catch (_) { settingsExtCall = null; }
    const anticallOn = settingsExtCall ? settingsExtCall.__getSetting('anticall') !== false : true;
    ...
    for (const call of inboundCall) {
      if (call.status === "offer") {
        try {
          ...
          const msgText = settingsExtCall ? settingsExtCall.__getSetting('anticallmessage') : '';
          if (msgText) {
            try { await socket.sendMessage(call.from, { text: msgText }); } catch (_) {}
          }
        } catch (e) {
  ...
  socket.ev.on("group-participants.update", async (update) => {
    try {
      ...
      if (update.action === 'add') {
        ...
        // ✅ FIX: antipromote/antidemote were toggleable (groupguard.js) but
        // never actually checked anywhere — pure placebo like the other
        // settings audited this pass. Implemented as an in-chat notice
        // (who got promoted/demoted) rather than auto-reverting the
        // action: there's no separate "auto-revert" setting exposed to
        // the owner, and silently undoing another admin's action risks a
        // promote/demote fight-loop between the bot and a human admin.
        try {
          const settingsExt = require('./plugins/settings-ext.js');
          const key = update.action === 'promote' ? 'antipromote' : 'antidemote';
          if (settingsExt.__getSetting(key)) {
            const who = (update.participants || []).map(p => `@${p.split('@')[0]}`).join(', ');
            const verb = update.action === 'promote' ? 'promoted to admin' : 'demoted from admin';
            await socket.sendMessage(update.id, { text: `👮 ${who} was just ${verb}.`, mentions: update.participants || [] });
          }
        } catch (_) {}
      }
    } catch (e) {
  ...
  socket.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (isStatus) {
        try {
          const settingsExt = require('./plugins/settings-ext.js');
          // ✅ NEW: cache text statuses so a later revoke (status deleted
          // before its 24h expiry) can be recovered — see statusantidelete
          // listener below, next to the message antidelete one.
          if (settingsExt.__getSetting('statusantidelete') && !msg.message?.protocolMessage) {
            cacheRecentMessage(msgId, { msg, sender: msg.key.participant || sender, name, senderJid: msg.key.participant || sender, isStatus: true });
          }
          if (settingsExt.__getSetting('autoreadstatus')) {
            await socket.readMessages([msg.key]);
          }
          if (settingsExt.__getSetting('autolikestatus')) {
            // ✅ FIX: statusemojis was configurable in the admin panel but
            // never actually read — every status got a hardcoded ❤️
            // regardless of what the owner set. Now picks randomly from
            // the configured pool (falls back to ❤️ if unset/empty).
            const pool = (settingsExt.__getSetting('statusemojis') || '❤️')
      ...
      // ── NEW: activity/leveling tracker — every real incoming group message
      // bumps the sender's message count (plugins/leveling.js), which also
      // fixes .rank/.leaderboard/.top (ported_group.js), whose counter was
      // never actually being incremented anywhere. Fire-and-forget: never
      // awaited, wrapped so a failure here can't affect message handling.
      if (!msg.key.fromMe && isGroup) {
        try {
          require('./plugins/leveling.js').__trackActivity({ sock: socket, chatId: sender, userId: senderJid });
      ...
      // ✅ FIX: scoped to isThisOwnerSession — this used to fire on EVERY
      // session (any customer's number running this codebase), when it
      // should only ever introduce Henry personally on Henry's own number.
      if (isThisOwnerSession && !isGroup && !isBotAdmin && !isStatus && !msg.message?.reactionMessage && !msg.message?.protocolMessage) {
        try {
      ...
      // ── Free self-service ".pair" chat linking ──────────────────────────────
      // Anyone messaging this bot can link their OWN number as a brand new
      // bot session — right here in chat, no payment, no admin approval,
      // no website needed.
      if (!isGroup && body) {
      ...
      if (body.startsWith(CMD_PREFIX)) {
        ...
        if (allCommands[cmd]) {
          ...
          // ✅ FIX: autotype/autorecord were configurable in the admin panel
          // (and .setautotype/.setautorecord existed) but nothing ever read
          // them back — pure placebo, like the other settings-ext toggles
          // fixed this pass. Now they actually show a brief typing/recording
          // indicator before a command's reply, matching what their own
          // descriptions in settings-ext.js always said they'd do.
          try {
            const settingsExtIndicator = require('./plugins/settings-ext.js');
            if (settingsExtIndicator.__getSetting('autorecord')) {
              await socket.sendPresenceUpdate('recording', sender);
            } else if (settingsExtIndicator.__getSetting('autotype')) {
              await socket.sendPresenceUpdate('composing', sender);
            }
          } catch (_) {}
          try {
            ...
            try {
              // ✅ FIX: antibug was configurable but never read — command
              // errors always leaked the raw exception message to whoever
              // typed the command, regardless of the setting. Now, when
              // antibug is on, non-admins get a generic failure message;
              // the owner/co-owner/sub-admins still see the real error
              // (they're the ones who'd need it to fix something), and the
              // full detail always still lands in the console + activity log.
              const settingsExtBug = require('./plugins/settings-ext.js');
              const hideDetail = settingsExtBug.__getSetting('antibug') && !isOwner && !isCoOwner && !isSubAdmin;
              await socket.sendMessage(sender,
                { text: hideDetail ? `❌ *.${cmd}* failed. Try again in a moment.` : `❌ Error in .${cmd}: ${e.message}` },
                { quoted: msg }
              );
            } catch (_) {}
          }
