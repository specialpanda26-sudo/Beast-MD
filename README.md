...
- **🛡️💳 Bot Panel: per-user Bot Health + Buy Subscription Time** — the `/panel` (customer-facing) panel now shows *that customer's own* linked session's live ban-risk, warm-up day, online status, and subscription expiry — separate from `/admin`'s aggregated "worst session" view, since every customer's own session differs. A new **Buy Subscription Time** section lets them extend their bot's activation days straight from their existing kesh wallet balance (`/api/subscription/buy`), no admin approval needed — this is deliberately **not** the `.pricing`/`.setprice` config-reselling flow, it's paying to keep their own bot's activation alive. Price-per-day is admin-configurable from `/admin → Activation` (defaults to 10 kesh/day). Also fixed along the way: the panel's wallet balance and top-up list never actually rendered — they were reading `data.balance`/`data.payments`, but the backend returns `data.credits`/`data.recent_payments`.
